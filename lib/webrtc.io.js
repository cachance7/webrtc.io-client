...
(function() {
    ...
    //rtc.init = function(initcfg, callback) {
    //    // Allows client to ask for room initialization
    //    rtc.socket.emit('initialize', initcfg);
    //    rtc.socket.on('ready', function(config){
    //        callback(config);
    //    });
    //};

    rtc.join = function(config, callback) {
        rtc.socket.emit('join_room', config, function(err, peerData) {
            if(err) {
                console.error(err);
            } else {
